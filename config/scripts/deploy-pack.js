...
const assets = 'web/app/themes/crean/assets'
...
sh.find('./web/app')
  .filter(file => file.match(/\.mo$/) && (file.match('plugins/crean') || file.match('themes/crean')))
  .forEach(lang => {
    sh.cp(lang, path.join('build', lang))
  })

// Replace version string (vDEV) in functions.php for cache busting
sh.sed('-i', 'vDEV', version, path.resolve('./build/web/app/themes/crean/functions.php'))
